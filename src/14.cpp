...
public:
    ...
    void random_init (int _h, int _w) {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = i+1; j < N; j++) {
                if (bases[inds[i]].h*bases[inds[i]].w < bases[inds[j]].h*bases[inds[j]].w) {
                    swap(inds[i], inds[j]);
                }
            }
        }
        if (engine.rand()%2) {
            if (engine.rand()%2) {
                for (int i = 0; i < N; i++) {
                    for (int j = i+1; j < N; j++) {
                        if (bases[inds[i]].h < bases[inds[j]].h || (bases[inds[i]].h ==bases[inds[j]].h && bases[inds[i]].w < bases[inds[j]].w)) {
                            swap(inds[i], inds[j]);
                        }
                    }
                }
                ...
                for (int i = 0; i < N; i++) {
                    for (int j = i+1; j < N; j++) {
                        if (bases[inds[i]].w < bases[inds[j]].w || (bases[inds[i]].w ==bases[inds[j]].w && bases[inds[i]].h < bases[inds[j]].h)) {
                            swap(inds[i], inds[j]);
                        }
                    }
                }
    ...
    void polish () {
        double start_time = getTime(start_cycle), end_time = 9.8;
        double start_temp = 100, end_temp = 0.1;
        double time_limit = end_time - start_time;
        init_que();
        int loop = 0;
        while (getTime(start_cycle) < end_time) {
            ...
            // double temp = start_temp + (end_temp - start_temp) * (getTime(start_cycle)-start_time) / time_limit;
            // double prob = exp((score - nows) / temp);

            if (score > nows) {
            // if (prob > (engine.rand()%inf)/(double)inf) {
                ph[v] = hp; pw[v] = wp;
            }
            for (int h1 = ph[v]; h1 < ph[v]+bases[v].h; h1++) {
                for (int w1 = pw[v]; w1 < pw[v]+bases[v].w; w1++) {
    ...
    void random_exchange () {
        ...
        int ratio = min(N-3, (int)(N*getTime(start_cycle)/9.5));
...
void solve() {
    ...
    while (getTime(start_cycle) < 4.5) {
        ...
        if (ans.loss_score*(1-P) < ans.comp_score*P*0.01) {
            nh = ans.h+engine.rand()%5-5;
            nw = ans.w+engine.rand()%5-5;
        }
        if (engine.rand()%2) swap(nh, nw);
        */
        int S = ans.w*ans.h;
        int nh = MINH + engine.rand()%(100-MINH);
        int nw = (S-1) / nh + engine.rand()%11 - 5;
