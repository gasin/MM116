        ...
        for (int _i = 0; _i < N; _i++) {
            ...
            if (dir_rand) {
                ...
                for (int j = 0; j <= h-bases[i].h; j++) {
                    for (int k = 0; k <= w-bases[i].w; k++) {
                        ...
                        for (int h1 = j; h1 < j+bases[i].h; h1++) {
                            for (int w1 = k; w1 < k+bases[i].w; w1++) {
                                nows += prec[h1][w1][bases[i].val[h1-j][w1-k]];
                                /*
                                int qs = que_si[h1][w1];
                                int bv = bases[i].val[h1-j][w1-k];
                                if (qs == 0) continue;
                                nows += ((bv-val[h1][w1]>=0)?(bv-val[h1][w1]):max(0,val2[h1][w1]-bv));
                                */
                            }
                            if (nows >= score) break;
                        }
            ...
            for (int h1 = h_pos; h1 < h_pos+bases[i].h; h1++) {
                for (int w1 = w_pos; w1 < w_pos+bases[i].w; w1++) {
                    ...
                    for (int k = 0; k < 26; k++) {
                        if (k <= val2[h1][w1]) prec[h1][w1][k] = val2[h1][w1]-k;
    ...
    void random_exchange (double start_time, double end_time) {
        ...
        for (int i = 0; i < h; i++) {
            for (int j = 0; j < w; j++) {
                ...
                for (int k = 0; k < 26; k++) {
                    if (k <= val2[i][j]) prec[i][j][k] = val2[i][j]-k;
