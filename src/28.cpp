...
public:
    ...
    void random_init (int _h, int _w) {
        ...
        for (int _i = 0; _i < N; _i++) {
            ...
            if (dir_rand) {
                for (int j = 0; j <= h-bases[i].h; j++) {
                    int k_rand = engine.rand()%2;
                    for (int _k = 0; _k <= w-bases[i].w; _k++) {
                        int k = k_rand?_k:w-bases[i].w-_k;
                        int nows = 0;
                        if (score == 0) break;
                        for (int h1 = j; h1 < j+bases[i].h; h1++) {
                            for (int w1 = k; w1 < k+bases[i].w; w1++) {
                ...
                for (int k = 0; k <= w-bases[i].w; k++) {
                    int j_rand = engine.rand()%2;
                    for (int _j = 0; _j <= h-bases[i].h; _j++) {
                        int j = j_rand?_j:h-bases[i].h-_j;
                        int nows = 0;
                        if (score == 0) break;
                        for (int h1 = j; h1 < j+bases[i].h; h1++) {
                            for (int w1 = k; w1 < k+bases[i].w; w1++) {
