...
public:
    ...
    void random_init (int _h, int _w) {
        ...
        // if (h*w < 150) {
            /*
            double nt = getTime(start_cycle);
            int hoge = 0;
            while (getTime(start_cycle) < nt+0.1) {
                hoge++;
                random_exchange(nt, nt+0.1);
            }
            */
            //cerr << hoge << endl;
        //}
    ...
    void polish (double start_time, double end_time) {
        double start_temp = 3.0, end_temp = 0.1;
        init_que();
        int loop = 0;
        while (getTime(start_cycle) < end_time) {
            ...
            double temp = start_temp + (end_temp-start_temp) * (getTime(start_cycle)-start_time) / (end_time-start_time);
    ...
    void random_exchange (double start_time, double end_time) {
        ...
        int ratio = min(N-2, (int)(N*(getTime(start_cycle)-start_time)/(end_time-start_time)));
...
void solve() {
    ...
    while (getTime(start_cycle) < 4.5) {
        ...
        if (engine.rand()%2) {
            while (nh < MINH || nh >= 100 || nw < MINW || nw >= 100) {
                int S = ans.w*ans.h;
                nh = MINH + engine.rand()%(100-MINH);
    ...
    if (ans.loss_score == 0) {
        ...
        ans.polish(mid_time, 9.8);
        int loop2 = 0;
        double start_temp = 10.0, end_temp = 1.0;
        while (getTime(start_cycle) < 9.5) {
            loop2++;
            STATE state;
            state.copy(ans);

            state.random_exchange(mid_time, 9.5);

            double temp = start_temp + (end_temp-start_temp) * (getTime(start_cycle)-mid_time)/(9.5-mid_time);
            double prob = exp((ans.loss_score-state.loss_score)/temp);
